...
def test_eval_metrics_match_spec():
    ...
    b0, b1, b2 = PREFIX_BUDGETS
    def fve(B, l):
        return round(0.2 + 0.001 * B + 0.005 * l, 5)
    cells = {f"{B},{l}": {"fve": fve(B, l)} for B in PREFIX_BUDGETS for l in TARGET_LAYERS}
    exp_m = (fve(b0, 24)
             + sum(fve(b1, l) for l in (23, 24, 25)) / 3.0
             + sum(fve(b2, l) for l in TARGET_LAYERS) / len(TARGET_LAYERS)) / 3.0
    assert abs(m_scheduled(cells) - exp_m) < 1e-9
    g = stage_gains(cells)
    assert abs(g["G_local"] - sum(fve(b1, l) - fve(b0, l) for l in (23, 25)) / 2.0) < 1e-9
    assert abs(g["G_outer"] - sum(fve(b2, l) - fve(b1, l) for l in (20, 22, 26, 28)) / 4.0) < 1e-9
