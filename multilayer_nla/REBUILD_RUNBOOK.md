...
## One-shot automation (fresh box, everything on HF)

`scripts/recreate_sweep.sh` does §1 (download bank + the 8 adapters) and §2
(`rebuild_datasets.sh`) in one resumable pass — use it instead of the manual steps below if you
just want the datasets + frozen models on disk:
```bash
DATA=/data/mlnla bash multilayer_nla/scripts/recreate_sweep.sh
# then: bash multilayer_nla/scripts/run_mean_input.sh   (the averaged-input experiment)
```
**The 8 trained adapters are on the model repo `senku21x/qwen3-8b-nla-multilayer-sweep`**
(`ar/iter_0003000`, `ar_L24`, `av_<cond>/iter_0001000` ×6) — DOWNLOAD them and skip §3
(training) entirely; only the new `av_mean` is trained fresh.
...
### 2a. Download (fast — the bank IS published)
**CONFIRMED:** the dataset repo `senku21x/qwen3-8b-nla-multilayer-L19-29` holds the bank at its
**root** — `{av_sft,ar_sft,rl}.shard{00..07}of08.parquet` (8 shards each) — alongside the
`results/`, `sweep/`, `sweep_eval/` folders. Pull just the bank:
```bash
export HF_HUB_ENABLE_HF_TRANSFER=1   # fast multi-threaded download
...
```
(The repo's `sweep/` folder is a CACHE of the built `$SWEEP` parquets — regenerable by §2, so
rebuild rather than trust it. `results/` + `sweep_eval/` are eval outputs, reference only.)
...
## 4b. Averaged-input (`mean`) condition — the new ablation

A 7th condition that mean-pools the input layers into ONE k=1 slot (`av_in_0 =
mean(L23,L24,L25)`) instead of three distinct slots, reconstructing the SAME fixed
[L23,L24,L25] target with the frozen 3-tap AR (and the L24-only AR). It's a mean-pool ablation
of `local`: it should land between `single` (39.1) and `local` (41.6) on test. One script does
build + train (wandb `av-mean`) + the eval cuts, reusing the frozen ARs:
```bash
AR=$CKPT/ar_3tap_bs256e_3k/iter_0003000  AR_L24=$CKPT/ar_l24only/iter_0003000 \
  bash multilayer_nla/scripts/run_mean_input.sh
```
Datasets are built via `build_sweep --av-pool` (k=1 mean slot) and gated by `--mode
verify-pool` (asserts `av_in_0 == mean(L23,24,25)`, not a copy). `mean` is intentionally NOT in
`select_and_report`'s condition list — it reuses the already-selected AR-3000/AV-1000, so it is
never part of checkpoint selection. To fold it into the paired contrasts, add `mean` to
`analyze_sweep`'s `--best-conds` and compare `mean−single` / `mean−local` (paired bootstrap over
shared `(doc_id, src_row_id)`). Caveat on reading it: `mean ≈ local` is ambiguous (no unique
cross-layer content OR a verbalizer too weak to exploit distinct slots); `mean ≈ single` is the
cleaner signal that the positional slots did the work.
