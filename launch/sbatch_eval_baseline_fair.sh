#!/bin/bash
#SBATCH --job-name=qwen3_val_baseline_fair
...
python launch/eval_av_val_loss.py \
  --ckpt-dir /workspace-vast/celeste/nla-ckpts/qwen3_8b_L24_av_sft_baseline_fair/iter_0001000/hf \
  --val-parquet $DATA/av_val.parquet \
  --sidecar $DATA/av_val.parquet \
  --injection embedding_replace \
  --injection-scale sqrt_d_model \
  --max-rows 500 \
  --output-json /workspace-vast/celeste/nla-experiments/logs/val_baseline_fair.json
